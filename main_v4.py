"""
Orbit Wars - Competitive Agent v4
===================================
...
Improvements in v4 (based on v2):
  - Multi-expansion per turn: Each planet can launch fleets to multiple neutrals if it has ships.
  - Better surplus forwarding: Don't just send to one front, distribute to all nearby attack points.
  - Tighter travel-time estimation for enemy planets (but not for neutrals).
  - Orbit prediction and Sun avoidance from v2 preserved.
"""
...
def agent(obs):
    ...
    for src in sorted(my_p, key=lambda p: -avail(p)):
        # Multi-target expansion from one source

...
def make_difficulty_agent(base_module, difficulty: str):
    """
    Wrap base_module.agent with temporarily overridden LEARNED_PARAMS so we can
    pit the agent against harder/easier versions of itself in self-play.
    Params are saved and restored around each call so games stay sequential-safe.
    """
    param_overrides = {
        # plays passively — needs overwhelming advantage before attacking
        "passive": {
            "aggressive_ship_ratio": 3.0,
            "min_attack_avail": 15,
            "consolidate_avail": 30,
        },
        # plays aggressively — attacks with smaller margins, consolidates faster
        "aggressive": {
...
def run_single_game(main_agent, opponent="random", seed: Optional[int] = None) -> Tuple[List, int, List]:
    ...
    opponent can be a string ("random") or any callable agent.
    """
...
def run_multiple_games(n_games: int = 20, opponents=None) -> Tuple[pd.DataFrame, List]:
    """
    Run n_games with a rotating opponent schedule.
    Default schedule: random × 2, self-play, passive self-play, aggressive self-play.
    """
...
def main():
    ...
    if not HAS_KAGGLE:
        ...
        df, ship_series = run_multiple_games(
            n_games=args.games,
            # None → default schedule: random × 2, self-play, passive, aggressive
        )

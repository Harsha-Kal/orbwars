...
PARAMS_PY_PATH = ROOT / "learned_params_output.py"
PARAMS_JSON_PATH = ROOT / "learned_params_output.json"
...
def format_params_block(params: Dict) -> str:
    """Return a copy-pasteable LEARNED_PARAMS block."""
...
def write_params_outputs(params: Dict) -> None:
    """Write learned params as Python and JSON for manual replacement."""
    py_text = (
        "# Auto-generated by learn_strategy.py.\n"
        "# Copy this block into main.py between the LEARNED_PARAMS markers.\n\n"
        f"{_PARAMS_MARKER_START}\n"
        f"{format_params_block(params)}\n"
        f"{_PARAMS_MARKER_END}\n"
    )
    PARAMS_PY_PATH.write_text(py_text)
    PARAMS_JSON_PATH.write_text(json.dumps(params, indent=2, sort_keys=True) + "\n")
    print(f"  [Params] Wrote copy-paste block to {PARAMS_PY_PATH}")
    print(f"  [Params] Wrote JSON params to {PARAMS_JSON_PATH}")
...
def main():
    ...
    # ── Step 8: Write params output files ──────────────────────────────────────
    write_params_outputs(params)
